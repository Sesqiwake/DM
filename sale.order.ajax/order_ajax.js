...
(function () {
	...
	BX.Sale.OrderAjaxComponent = {
		...
		getData: function (action, actionData) {
			...
			data.order.processAction = this.processAction;
			this.processAction = '';
			return data;
		},
		...
		getAllFormData: function () {
			var form = BX('bx-soa-order-form'),
				prepared = this.prepareFormExtend(form),
				i;
		...
		customSaveProp: function () {
			if (this.result.ORDER_PROP && this.result.ORDER_PROP.properties) {
				var props = this.result.ORDER_PROP.properties, i, prop;
				for (i = 0; i < props.length; i++) {
					if (props[i].NAME == 'Адрес доставки' || props[i].NAME == 'Контактное лицо' || props[i].NAME == 'Телефон') {
						// prop = this.orderBlockNode.querySelector('div[data-property-id-row="' + props[i].ID + '"]');
						props[i].VALUE = this.orderBlockNode.querySelector("#soa-property-" + props[i].ID).value;
						this.savedVariableProp[props[i].ID] = this.orderBlockNode.querySelector("#soa-property-" + props[i].ID).value;
					}
				}
			}
		},
		...
		editActiveRegionBlock: function (activeNodeMode) {
			...
			BX.addClass(regionContent, 'form order__buyer');

			this.getErrorContainer(regionContent);

			this.getPersonTypeControl(regionContent);

			this.getProfilesControl(regionContent);

			this.editPropsItems(regionContent);

			// this.getDeliveryLocationInput(regionContent); 
		...
		getDeliveryLocationInput: function (node) {
			...
			if (location && location[0] && location[0].output) {
				...
				labelHtml = '<label class="form__name" for="soa-property-' + locationId + '">'
		...
		getPersonTypeControl: function (node) {
			...
			if (personTypesCount > 2) {
				...
				for (i in this.result.USER_PROFILES) {
					if (this.result.USER_PROFILES[i].CHECKED == 'Y') {
						active = this.result.USER_PROFILES[i].PERSON_TYPE_ID
						break;
					}
				}
				...
				for (i in this.result.PERSON_TYPE) {
					if (this.result.PERSON_TYPE.hasOwnProperty(i)) {
						...
						if (currentType.ID == active) {
							currentType.CHECKED = 'Y';
						} else {
							currentType.CHECKED = 'N';
						}
				...
				node.appendChild(BX.create('SELECT', {
					props: { name: 'PERSON_TYPE', className: 'form-control', id: 'person_type_controll' },
					children: options,
		...
		getProfilesControl: function (node) {
			...
			var selectBlock = BX.create('DIV', {
				props: { className: 'select js-select select--order', id: 'select-buyer' }
			});
			...
			if (profilesLength && this.params.ALLOW_USER_PROFILES === 'Y') {
				...
				if (profilesLength > 1 || this.params.ALLOW_NEW_PROFILE === 'Y') {
					label = BX.create('LABEL', { props: { className: 'form__name' }, html: 'На кого будет оформлен заказ' });

					var parentUl = BX.create('UL', {
						props: { className: 'select__list js-select-list' }
					});
					...
					for (i in this.result.USER_PROFILES) {
						if (this.result.USER_PROFILES.hasOwnProperty(i)) {
							...
							if (this.result.USER_PROFILES[i].CHECKED == 'Y') {
								selectedItem = BX.create('DIV', {
									props: { className: 'select__head js-select-btn' },
									data: { value: '#item_' + this.result.USER_PROFILES[i].ID },
									children: [
										BX.create('SPAN', {
											props: { className: 'select__text' },
											text: this.htmlspecialchars_decode(this.result.USER_PROFILES[i].NAME)
										})
									]
								})
							}

							parentUl.appendChild(BX.create('LI', {
								props: {
									className: 'select__item js-select-option' + (this.result.USER_PROFILES[i].CHECKED ? ' active' : ''),
									value: this.result.USER_PROFILES[i].ID,
									selected: this.result.USER_PROFILES[i].CHECKED
								},
								dataset: { person: this.result.USER_PROFILES[i].PERSON_TYPE_ID },
								children: [
									BX.create('SPAN', {
										props: { className: 'select__text' },
										text: this.htmlspecialchars_decode(this.result.USER_PROFILES[i].NAME)
								...
								events: {
									click: function (e) {
										...
										document.querySelector("#person_type_controll_2").value = e.target.closest(".js-select-option").value;
										document.querySelector("#person_type_controll").value = e.target.closest(".js-select-option").dataset.person;
										// $('#person_type_controll_2').val($(e.target).parent().attr('value'));
										// $('#person_type_controll').val($(e.target).parent().attr('data-person'));
										// $('.js-ID-prop').val($(e.target).parent().attr('value'));
										BX('profile_change').value = 'Y';

										_this.processAction = 'selectProfile';
										setTimeout(function () {
											_this.sendRequest();
										}, 500)
									}
					...
					selectBlock.appendChild(selectedItem);
					selectBlock.appendChild(parentUl);
					...
					input = BX.create('SELECT', {
						props: { className: 'form-control', name: 'PROFILE_ID', id: 'person_type_controll_2' },
						children: options,
						events: {
							change: BX.delegate(function () {
								BX('profile_change').value = 'Y';
					...
					node.appendChild(
						BX.create('DIV', {
							props: { className: "form__group" },
							children: [label, selectBlock, profileChangeInput, input]
						})
					);
				}
				else {
		...
		editPaySystemItems: function (paySystemNode) {
			...
			var paySystemItemsContainer = BX.create('DIV', {
				props: { className: 'form__group' }, children: [
					BX.create('LABEL', { props: { className: 'form__name' }, text: 'Способ оплаты' })
				]
			}), paySystemItemNode, i;

			var selectBlock = BX.create('DIV', {
				props: { className: 'select js-select select--order' }
			});

			paySystemItemsContainer.appendChild(selectBlock);
			...
			for (i = 0; i < this.paySystemPagination.currentPage.length; i++) {
				var item = this.paySystemPagination.currentPage[i];
				var paymentId = parseInt(item.ID);

				if (item.CHECKED == 'Y') {
					selectedItem = BX.create('DIV', {
						props: { className: 'select__head js-select-btn' },
						data: { value: '#item_' + paymentId },
						children: [
							BX.create('SPAN', {
								props: { className: 'select__text' },
								text: item.NAME
				...
				parentUl.appendChild(BX.create('LI', {
					props: {
						className: 'select__item js-select-option' + (item.CHECKED == 'Y' ? ' active' : ''),
						value: paymentId,
						selected: item.CHECKED == 'Y'
					},
					children: [
						BX.create('SPAN', {
							props: { className: 'select__text' },
					...
					events: {
						click: function (e) {
							console.clear();
							_this.customSaveProp();
							_this.selectPaySystem(e);
						}
					}
				}));
		...
		selectPaySystem: function (event) {
			...
			$('input[name="PAY_SYSTEM_ID"]').each(function () {
				...
				$(this).parent().removeClass('bx-selected');
			});

			var number = $(target).parent().attr('value');

			$('#ID_PAY_SYSTEM_ID_' + number).prop('checked', 'checked').parent().addClass('bx-selected');

			this.processAction = 'selectServices';
			this.sendRequest();
		},
		...
		editActiveDeliveryBlock: function (activeNodeMode) {
			...
			BX.addClass(deliveryContent, 'form order__buyer');
			...
			if (currentDelivery.STORE.length > 0) {
				if (currentDelivery.ID == 7) {
					this.createStoreCitySelect(deliveryContent, currentDelivery.STORE);
				}

				this.createStoreSelect(deliveryContent, currentDelivery.STORE);

				if (currentDelivery.ID == 7) {
					this.actualCityStores();
				}
			}

			this.editDeliveryPropsItems(deliveryContent);

			this.editPaySystemItems(deliveryContent);

		},

		actualCityStores: function (e) {
			if (e) {
				var target = $(e.target);

				if ($(target).hasClass('select__text')) {
					city = $(target).html();
				} else {
					var city = $(target).find('.select__text').html();
				}
			} else {
				var city = $('.js-city .active .select__text').html();
			}

			$('.js-stores li').hide().removeClass('active');
			$('.js-stores li[data-city=' + city + ']').show();

			var store = $($('.js-stores li:visible')[0]);
			$(store).addClass('active');
			$(store).parents('.select').find('.select__head .select__text').html($(store).find('.select__text').html());
			$('#BUYER_STORE').val($(store).attr('value'));
		},

		createStoreSelect: function (node, stores) {
			var deliveryItemsContainer = BX.create('DIV', {
				props: { className: 'form__group' }, children: [
					BX.create('LABEL', { props: { className: 'form__name' }, text: 'Пункт самовывоза' })
				]
			}),
				deliveryItemNode, k;
			...
			deliveryItemsContainer.appendChild(selectBlock);

			var parentUl = BX.create('UL', {
				props: { className: 'select__list js-select-list js-stores' }
			});
			...
			for (k = 0; k < stores.length; k++) {
				var item = this.result.STORE_LIST[stores[k]];
				var deliveryId = parseInt(item.ID);

				if (!flag) {
					selectedItem = BX.create('DIV', {
						props: { className: 'select__head js-select-btn' },
						data: { value: '#item_' + deliveryId },
						children: [
							BX.create('SPAN', {
								props: { className: 'select__text' },
								text: item.ADDRESS
				...
				parentUl.appendChild(BX.create('LI', {
					props: {
						className: 'select__item js-select-option' + (!flag ? ' active' : ''),
						value: deliveryId,
						selected: !flag
					},
					dataset: { city: item.DESCRIPTION },
					children: [
						BX.create('SPAN', {
							props: { className: 'select__text' },
							text: item.ADDRESS
					...
					events: {
						click: function (e) {
							$('#BUYER_STORE').val($(e.target).parent().attr('value'));
						}
					}
				}));
			...
			node.appendChild(deliveryItemsContainer);
		},

		createStoreCitySelect: function (node, stores) {
			var deliveryItemsContainer = BX.create('DIV', {
				props: { className: 'form__group' }, children: [
					BX.create('LABEL', { props: { className: 'form__name' }, text: 'Город' })
				]
			}),
				deliveryItemNode, k;
			...
			var parentUl = BX.create('UL', {
				props: { className: 'select__list js-select-list js-city' }
			});
			...
			var cities = [];
			for (k = 0; k < stores.length; k++) {
				cities.push(this.result.STORE_LIST[stores[k]].DESCRIPTION);
			}
			...
			for (k = 0; k < cities.length; k++) {
				...
				parentUl.appendChild(BX.create('LI', {
					...
					events: {
						click: BX.proxy(this.actualCityStores, this)
					}
				}));
		...
		editDeliveryItems: function (deliveryNode) {
			...
			var deliveryItemsContainer = BX.create('DIV', {
				props: { className: 'form__group' }, children: [
					BX.create('LABEL', { props: { className: 'form__name' }, text: 'Способ доставки' })
				]
			}),
				deliveryItemNode, k;
			...
			this.editDeliveryInfo(deliveryItemsContainer);
		},
		...
		editDeliveryInfo: function (deliveryNode) {
			...
			var deliveryInfoContainer = BX.create('DIV', { props: { className: 'order-delivery' } }),
			...
			if (currentDelivery.PRICE >= 0) {
				if (currentDelivery.ID == "10") {
					price = BX.create('SPAN', {
						props: { className: 'order-delivery__price' },
						text: 'Стоимость доставки уточняется менеджером магазина'
					});
				} else {
					price = BX.create('SPAN', {
						props: { className: 'order-delivery__price' },
						text: 'Стоимость: ' + this.getDeliveryPriceNodes(currentDelivery)
					});
			...
			deliveryInfoContainer.appendChild(
				BX.create('DIV', {
					props: { className: 'order-delivery__item active' },
					children: [price]
				})
			);
			...
			if (currentDelivery.DESCRIPTION) {
				deliveryInfoContainer.appendChild(BX.create('DIV', {
					props: { className: 'order-delivery__text' },
			...
			if (currentDelivery.ID == "16") {
				console.log("exali")
				deliveryNode.appendChild(BX.create('button', {
					props: {
						className: 'btn btn-reset btn--secondary',
						id: "delivery_select_pvz"
					},
				}));
			}
		...
		selectDelivery: function (event) {
			...
			$('#ID_DELIVERY_ID_' + number).prop('checked', 'checked').parent().addClass('bx-selected');
		...
		editActivePropsBlock: function (activeNodeMode) {
			...
			BX.addClass(propsContent, 'form order__buyer');
		...
		editPropsItems: function (propsNode) {
			...
			while (group = groupIterator()) {
				...
				while (property = propsIterator()) {
					...
					if (parseInt(property.getGroupId()) == 18 || parseInt(property.getGroupId()) == 19 || parseInt(property.getGroupId()) == 20 || parseInt(property.getGroupId()) == 21) {
						this.getPropertyRowNode(property, propsNode, false, true);
					}
				}
			}
		},

		editDeliveryPropsItems: function (propsNode) {
			if (!this.result.ORDER_PROP || !this.propertyCollection)
				return;
			...
			while (group = groupIterator()) {
				...
				while (property = propsIterator()) {
					...
					if (parseInt(property.getGroupId()) != 18 && parseInt(property.getGroupId()) != 19 && parseInt(property.getGroupId()) != 20 && parseInt(property.getGroupId()) != 21) {
						this.getPropertyRowNode(property, propsNode, false, false);
					}
				}
			}
		},

		getPropertyRowNode: function (property, propsItemsContainer, disabled, allFields = false) {
			...
			if (disabled) {
				...
				BX.addClass(propsItemNode, "form__group");
			...
			switch (propertyType) {
				...
				case 'FILE':
					...
					break;
				case 'STRING':
					this.insertStringProperty(property, propsItemNode, disabled, allFields);
					break;
				case 'ENUM':
		...
		insertStringProperty: function (property, propsItemNode, disabled, allFields = false) {
			var prop, inputs, values, i, propContainer;
			property.appendTo(propsItemNode);
			propsItemNode.appendChild(BX.create('SPAN', {
				props: { className: 'form__error-text' }
			}));
			this.alterProperty(property.getSettings(), propsItemNode, allFields);
		...
		alterProperty: function (settings, propContainer, allFields = false) {
			...
			if (settings.PROPS_GROUP_ID == 54 || settings.PROPS_GROUP_ID == 55 || settings.PROPS_GROUP_ID == 56 || settings.PROPS_GROUP_ID == 57) {
				BX.addClass(propContainer, 'hidden');
			}

			if (allFields && settings.CODE != 'FULL_NAME' && settings.CODE != 'PHONE' && settings.CODE != 'EMAIL') {
				BX.addClass(propContainer, 'hidden');
			}
			...
			if (textNode) {
				...
				if (settings.CODE.includes('PHONE')) {
					textNode.setAttribute('type', 'tel');
				}

				if (settings.CODE.includes('EMAIL')) {
					textNode.setAttribute('type', 'mail');
					textNode.setAttribute('data-mask', 'email')
				}

				if (settings.CODE == 'BONUSES') {
					BX.addClass(textNode, 'js-bonuses-prop');
					$(textNode).val(this.result.APPLY_BONUSES > 0 ? this.result.APPLY_BONUSES : '');
				}
				if (settings.CODE == 'ID_COUNTER') {
					BX.addClass(textNode, 'js-ID-prop');
					if (document.querySelector('#select-buyer')) {
						var parent = document.querySelector('#select-buyer');
						textNode.value = parent.querySelector('.js-select-option.active').value;
					} else if (document.querySelector('[name=PROFILE_ID]')) {
						textNode.value = document.querySelector('[name=PROFILE_ID]').value;
					}
				}
			}
			...
			for (i = 0; i < inputs.length; i++) {
				...
				inputs[i].placeholder = settings.DESCRIPTION;
				BX.addClass(inputs[i], 'form__input mask__number');
			}

			inputs = propContainer.querySelectorAll('input[type=mail]');
			for (i = 0; i < inputs.length; i++) {
			...
			for (i = 0; i < inputs.length; i++) {
				inputs[i].placeholder = settings.DESCRIPTION;
				BX.addClass(inputs[i], 'form__textarea');
			}
			...
			if (settings.CODE == "DELIVERY_ADDRESS" ||
				settings.CODE == "DELIVERY_CONTACT" ||
				settings.CODE == "DELIVERY_PHONE") {
				this.savedVariableProp.forEach(function (element, index) {
					var elp = propContainer.querySelector("#soa-property-" + index);
					if (elp) {
						elp.setAttribute('value', element);
						elp.value = element;
				...
				if (this.activeSectionId == this.propsBlockNode.id) {
					...
					for (i = 0; i < tooltips.length; i++) {
						if (tooltips[i].getAttribute('data-state') == 'opened') {
							this.animateScrollTo(BX.findParent(tooltips[i], { className: 'form__group' }), 800, 50);
							break;
						}
					}
				}
		...
		isValidPropertiesBlock: function (excludeLocation) {
			...
			var props = this.deliveryBlockNode.querySelectorAll('.form__group[data-property-id-row]'),
				propsErrors = [],
		...
		showValidationResult: function (inputs, errors) {
			...
			var input0 = BX.type.isElementNode(inputs[0]) ? inputs[0] : inputs[0][0],
				formGroup = BX.findParent(input0, { tagName: 'DIV', className: 'form__group' }),
				label = formGroup.querySelector('label'),
				tooltipId, inputDiv, i;

			for (i = 0; i < inputs.length; i++) {
				inputDiv = BX.findParent(inputs[i], { tagName: 'DIV', className: 'form__group' });
				if (errors[i] && errors[i].length) {
					BX.addClass(inputDiv, 'form--invalid');
					$(inputDiv).find('span.form__error-text').html(errors[i]);
				}
				else
					BX.removeClass(inputDiv, 'form--invalid');
			}
		...
		prepareFormExtend: function(obForm, data) {
			data = (!!data ? data : {});
			var i, ii, el,
				_data = [],
		...
		editPropsComment: function (propsNode) {
			...
			label = BX.create('SPAN', {
				props: { className: 'counter-symbol' },
				html: 'Введено <span class="counter-num">0</span> из 1000 символов'
			});
			...
			input = BX.create('TEXTAREA', {
				props: {
					id: 'orderDescription',
					cols: '30',
					className: 'form__textarea counter-input',
		...
		editTotalBlock: function () {
			...
			for (var i in this.result.GRID.ROWS) {
				count += parseInt(this.result.GRID.ROWS[i].data.QUANTITY);
			}

			var topBlock = BX.create('DIV', {
				props: { className: 'payment-info__body' },
				children: [
					BX.create('H3', {
						props: { className: 'payment-info__title' },
						text: 'Данные о заказе:'
					}),
					BX.create('DIV', {
						props: { className: 'payment-info__item' },
						children: [
							BX.create('SPAN', {
								props: { className: 'payment-info__text' },
								text: 'Товаров в заказе:'
							}),
							BX.create('SPAN', {
								props: { className: 'payment-info__sum' },
								text: count + ' шт.'
							})
						]
					}),
			...
			topBlock.appendChild(this.createTotalUnit('Сумма без учёта скидки', priceHtml, params));
			...
			if (deliveryError) {
				...
				if (parseFloat(total.DELIVERY_PRICE) === 0) {
					if (curDelivery.ID == "10") {
						deliveryValue = this.params.MESS_PRICE_FREE_ESTIMATED;
					} else {
			...
			if (this.result.DELIVERY.length) {
				topBlock.appendChild(this.createTotalUnit(BX.message('SOA_SUM_DELIVERY'), deliveryValue, params));
			}
			...
			if (this.options.showPayedFromInnerBudget) {
				...
				finalBlock = this.createTotalUnit(BX.message('SOA_SUM_IT'), total.ORDER_TOTAL_PRICE_FORMATED, { total: true });
			}

			if (parseFloat(total.PAY_SYSTEM_PRICE) >= 0 && this.result.DELIVERY.length) {
				finalBlock.appendChild(this.createTotalUnit(BX.message('SOA_PAYSYSTEM_PRICE'), '~' + total.PAY_SYSTEM_PRICE_FORMATTED));
			}

			var _this = this;

			if (this.result.APPLY_BONUSES) {
				topBlock.appendChild(BX.create('DIV', {
					props: { className: 'payment-info__item' },
					children: [
						BX.create('SPAN', {
							props: { className: 'payment-info__text' },
						...
						BX.create('SPAN', {
							props: { className: 'payment-info__sum' },
							text: this.result.APPLY_BONUSES + ' руб.'
						})
					]
				}));
			}
			...
			if (!this.result.SHOW_AUTH) {
					...
					children: [
						...
						BX.create('DIV', {
							...
							children: [
								BX.create('SPAN', {
									props: { className: 'payment-info__text' },
									text: 'Доступно к списанию:'
								}),
								BX.create('SPAN', {
									props: { className: 'payment-info__sum' },
									text: this.result.BONUSES
								})
							]
						}),
						BX.create('DIV', {
							...
							children: [
								BX.create('SPAN', {
									props: { className: 'basket-input' },
									children: [
										BX.create('INPUT', {
											props: { className: 'input-reset', placeholder: 'Введите количество', type: 'number', name: 'APPLY_BONUSES', value: this.result.APPLY_BONUSES > 0 ? this.result.APPLY_BONUSES : '' }
										})
									]
								}),
								BX.create('BUTTON', {
									props: {
										href: 'javascript:void(0)',
										className: 'btn btn-reset btn--secondary btn--bonuses',
										type: 'button'
									},
									html: 'Списать',
									events: {
										click: function () {
											$('.js-bonuses-prop').val($('input[name=APPLY_BONUSES]').val());
											_this.sendRequest();
										}
									}
								})
						...
						BX.create('DIV', {
							...
							children: [
								BX.create('SPAN', {
									props: { className: 'form__name' },
									text: 'Не более 10% от суммы заказа'
								})
							]
						}),
					]
				...
				this.totalInfoBlockNode.appendChild(topBlock);

				this.totalInfoBlockNode.appendChild(finalBlock);

				this.totalInfoBlockNode.appendChild(
					BX.create('BUTTON', {
						props: {
							href: 'javascript:void(0)',
							className: 'btn btn-reset btn--secondary btn-order-save'
						},
		...
		createTotalUnit: function (name, value, params) {
			var totalValue, className = 'payment-info__item';
			...
			if (params.total) {
				className = 'payment-info__final';

				return BX.create('DIV', {
					props: { className: className },
					children: [
						BX.create('SPAN', { props: { className: 'payment-info__final-text' }, text: name }),
						BX.create('SPAN', {
							props: {
								className: 'payment-info__final-text'
							},
							children: totalValue
						})
					]
				...
				return BX.create('DIV', {
					props: { className: className },
					children: [
						BX.create('SPAN', { props: { className: 'payment-info__text' }, text: name }),
						BX.create('SPAN', {
							props: {
								className: 'payment-info__sum'
							},
							children: totalValue
						})
					]
